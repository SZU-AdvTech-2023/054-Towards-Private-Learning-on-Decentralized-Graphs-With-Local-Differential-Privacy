...
from torch_geometric.nn import TransformerConv, GraphMultisetTransformer
...
class GAT(GNN):
    def __init__(self, input_dim, output_dim, hidden_dim, dropout):
        ...
        self.conv2 = GATConv(heads * hidden_dim, output_dim, heads=4, concat=False)
...
class GATv2(GNN):
    def __init__(self, input_dim, output_dim, hidden_dim, dropout):
        ...
        self.conv2 = GATv2Conv(heads * hidden_dim, output_dim, heads=4, concat=False)

class Transformer(GNN):
    def __init__(self, input_dim, output_dim, hidden_dim, dropout):
        super().__init__(dropout)
        heads = 6
        self.conv1 = TransformerConv(input_dim, hidden_dim, heads=heads, concat=True)
        self.conv2 = TransformerConv(heads * hidden_dim, output_dim, heads=4, concat=False)
...
class NodeClassifier(torch.nn.Module):
    def __init__(self,
                 input_dim,
                 num_classes,
                 model:                 dict(help='backbone GNN model', choices=['gcn', 'sage', 'gat', 'gatv2', 'transformer']) = 'gat',
        ...
        self.gnn = {'gcn': GCN, 'sage': GraphSAGE, 'gat': GAT, 'gatv2': GATv2, 'transformer':Transformer,}[model](
            input_dim=input_dim,
    ...
    def forward(self, data):
        ...
        # print("----------data.num--------：{}".format(data.num_classes))

        # 行归一化，就是一行加起来为1
        p_y_x = F.softmax(x, dim=1)                                                         # P(y|x')
        # p_y_x：softmax已经计算出了所有节点属于分别属于每一类的概率
        # print("----------p_y_x--------：{}".format(p_y_x))
        # print("----------p_y_x[0]--------：{}".format(p_y_x[0]))
        # p_yp_x = torch.matmul(p_y_x, data.T) if self.forward_correction else p_y_x          # P(y'|x')
        p_yp_x = p_y_x
        # print("----------p_yp_x--------：{}".format(p_yp_x))
        p_yt_x = self.y_prop(p_yp_x, data.adj_t)                                            # P(y~|x')
        # print("----------p_yt_x--------：{}".format(p_yt_x))
        # print("----------p_yt_x--------：{}".format(p_yt_x[0]))
    ...
    def validation_step(self, data):
        ...
        # print("----------------此处是测试-------------------")
        # print("data：{}".format(data))
        # print("data.T：{}".format(data.T))
        # print("T[0]：{}".format(data.T[0]))
        # print("p_y_x：{}".format(p_y_x))
        # print("p_y_x：{}".format(p_y_x[0]))
        # print("p_yp_x：{}".format(p_yp_x))
        # print("p_yt_x：{}".format(p_yt_x))
