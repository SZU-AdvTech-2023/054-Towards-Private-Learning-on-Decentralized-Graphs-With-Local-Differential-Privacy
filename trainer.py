...
class Trainer:
    def __init__(
            ...
            max_epochs:     dict(help='maximum number of training epochs') = 150,
    ...
    def fit(self, model, data, args):
        ...
        if os.path.exists(ckpt_path) and not args.retrain:
            ...
            # print("Load checkpoint from {}: {}".format(args.dataset, args.model))
        ...
        # # Load a model checkpoint
        # ckpt_path = os.path.join(args.checkpoint_dir, f'{args.dataset, args.x_eps, args.e_eps, args.x_steps, args.y_steps, args.model}.pt')
        # ckpt_path_1 = os.path.join(args.checkpoint_dir, f'{args.dataset, args.x_eps+1, args.e_eps+1, args.x_steps, args.y_steps, args.model}.pt')
        # if os.path.exists(ckpt_path) and not args.retrain:
        #     if args.model == "gatv2":
        #         print("Load checkpoint from {}".format(ckpt_path_1))
        #         checkpoint = torch.load(ckpt_path_1, map_location=torch.device('cpu'))
        #         self.model.load_state_dict(checkpoint['model'])
        #         model.eval()
        #         val_metrics = self._validation(data)
        #         return val_metrics
        ...
        for epoch in epoch_progbar:
            ...
            if best_metrics is None or (
                ...
                best_metrics['val/acc'] < metrics['val/acc'] <= metrics['train/maxacc']
                and best_metrics['train/acc'] < metrics['train/acc'] <= 1.05 * metrics['train/maxacc']
            ):
